...
function updateSearchCounter() {
  if (searchTerm === "") {
    ...
    $("#search-counter").text(`${searchMatches.length} found`).css("color", "#28a745");
  }
}
```
...
<div style="position: relative; width: 300px;">
  ...
  <button id="clear-search-btn" style="position: absolute; right: 5px; top: 5px; display: none;">
...
function navigateToMatch(direction) {
  ...
  $("#search-counter").text(`${currentMatchIndex + 1} of ${searchMatches.length}`);
}
...
@keyframes pulse {
  0%, 100% { box-shadow: 0 0 0 3px #007bff; }
...
function applyPropertyStatusFilter() {
  ...
  $(".node-card").each(function() {
    const visibleProps = $(this).find(".property-row:not(.hidden-by-filter)").length;
    if (visibleProps === 0) {
...
function applyEmptyValueFilter() {
  ...
  // Show count
  $("#empty-filter-count").text(hiddenCount > 0 ? `(${hiddenCount} hidden)` : "");
}
```
...
function showEmptyState() {
  const visibleCards = $(".node-card:not(.hidden-by-filter):not(.hidden-by-search)");
  
  if (visibleCards.length === 0) {
    let message = "No results found.";
    
    if ($("#search-input").val()) {
      message = "No matches found. Try different search terms or adjust filters.";
