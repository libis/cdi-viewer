...
**Assertions:**
```typescript
await expect(page.locator('.alert-success')).toContainText('Loaded: simple.jsonld');
...
await expect(page.locator('.property-row:has-text("description")')).toBeVisible();
await expect(page.locator('.add-property-dropdown option:has-text("description")')).not.toBeVisible();
```
...
**Assertions:**
```typescript
await page.setInputFiles('#local-file-input', 'fixtures/test-data/invalid-data.jsonld');
await expect(page.locator('#validation-status')).toContainText('Validating...', { timeout: 1000 });
await expect(page.locator('#validation-status')).toContainText('violations found', { timeout: 4000 });
await expect(page.locator('.property-row.invalid')).toHaveCount(expectedViolationCount);
```
...
await expect(page.locator('#validation-status')).not.toContainText('Validating...');
// Wait for debounce
await page.waitForTimeout(3500);
...
await expect(page.locator('#search-counter')).toContainText('1 of ' + expectedMatches);
...
await expect(page.locator('.node-card:not(.hidden-by-filter)')).toHaveCount(expectedInvalidCount);
...
await expect(page.locator('#namespace-table-body tr')).toHaveCount(expectedPrefixCount);
```
...
await expect(page.locator('#namespace-table-body tr:has-text("custom")')).toBeVisible();
```
...
await expect(page.locator('.node-card:not(.collapsed)')).toHaveCount(totalNodeCount);
```
